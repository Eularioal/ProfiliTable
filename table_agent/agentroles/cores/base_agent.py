...
# 项目内部导入
# =============================================================================
from table_agent.llm_callers.base import BaseLLMCaller
from table_agent.parsers.parsers import BaseParser
from table_agent.graphbuilder.message_history import AdvancedMessageHistory
from table_agent.promptstemplates.prompt_template import PromptsTemplateGenerator
from table_agent.states.base_state import MainState
from table_agent.utils.node_helpers import parse_react_output, observation_to_message, truncate_for_log
from table_agent.utils.utils import safe_exec_code
from table_agent.utils.utils import robust_parse_json, get_project_root
from table_agent.toolkits.tool_manager import ToolManager
from table_agent.utils.logger import get_logger
from table_agent.agentroles.cores.strategies import ExecutionStrategy
...
class BaseAgent(ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        try:
            ...
            name = tmp.role_name
            # 注册到 AgentRegistry
            from table_agent.agentroles.cores.registry import AgentRegistry
    ...
    def __init__(self, 
        ...
        if execution_config:
            ...
            # 创建执行策略
            from table_agent.agentroles.cores.strategies import StrategyFactory
    ...
    def parser(self) -> BaseParser:
        ...
        if self._parser is None:
            from table_agent.parsers import ParserFactory
    ...
    def parse_result(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            log.debug(f"[parse_result 解析结果] : {parsed}")
            return parsed
        except Exception as e:
    ...
    def get_llm_caller(self, state: MainState) -> BaseLLMCaller:
        ...
        """
        if self.use_vlm:
            from table_agent.llm_callers import VisionLLMCaller
    ...
    async def process_simple_mode(self, state: MainState, pre_tool_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 创建 LLM（不绑定工具）
        if state['llm_tracker']:
            llm = state['llm_tracker']
        else:
        ...
        try:
            ...
            answer_msg = await llm(messages)
            answer_text = answer_msg.content
            log.debug(f'LLM原始输出：{answer_text}')
    ...
    # =========================================================================
    # G. 执行模式 - ReAct_Table 模式 TODO: 探讨与ReAct合并的可能性
    # =========================================================================
    async def process_table_mode(self, state: MainState, pre_tool_results: Dict[str, Any]) -> Dict[str, Any]:
        """
        ReAct_Table 模式处理 - 基于 ReAct 框架的表格处理

        循环调用 LLM，执行 THINK → ACTION → OBSERVE 的模式，直到获得最终结果或达到最大步骤限制。
        ...
        """
        log.info("🔍🔍 开始 ReAct_Table 模式处理")
        # action为code形式，直接执行代码，但是注意state要提供原始数据路径/执行时间（加上？）
        # 初始化消息和跟踪，暂时不管理历史（按道理不压缩的话，整个react trace就是历史？）
        messages = self.build_messages(state, pre_tool_results)
        react_trace = []
        execution_time = state.get('execution_time', 0.0)
        for step in range(1, self.react_max_retries + 1):
            log.info(f"➡️ ReAct Step {step}/{self.react_max_retries}")
            # 调用 LLM
            if state.get('llm_tracker', None):
                llm = state['llm_tracker']
            else:
                llm = self.create_llm(state, bind_post_tools=False)
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    try:
                        stdout, exec_time = safe_exec_code(
                            ...
                            input_path=state["raw_table_paths"],
                        )
                        execution_time += exec_time
                        obs = {"status": "success", "stdout": stdout, "exec_time_sec": round(exec_time, 2)}
                    except Exception as e:
                        obs = {"status": "error", "stderr": str(e)}
                        log.error(f"💥 代码执行失败: {e}")

            # 记录 OBSERVATION 步骤
            react_trace.append({"step": len(react_trace) + 1, "type": "observation", "content": json.dumps(obs, ensure_ascii=False)})
            status_icon = "✅" if obs.get("status") == "success" else "❌"
            log.info(f"{status_icon} OBS: {truncate_for_log(obs.get('stdout', obs.get('stderr', '')))}")

            # 添加观察结果到消息
            messages.append({"role": "user", "content": observation_to_message(obs) + "你的代码必须是完全自包含的，可以独立执行。不要依赖之前的上下文或代码片段。"})
