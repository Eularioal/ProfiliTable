...
class TableStrategy(ExecutionStrategy):
    """表格处理模式策略"""
    
    async def execute(self, state: "MainState", **kwargs) -> Dict[str, Any]:
        log.info(f"[TableStrategy] 执行 {self.agent.role_name}，表格处理模式")
        
        # 执行前置工具获取结果
        pre_tool_results = await self.agent.execute_pre_tools(state)
        
        # 执行表格处理模式
        return await self.agent.process_table_mode(state, pre_tool_results)
...
class PlanSolveStrategy(ExecutionStrategy):
    ...
    async def execute(self, state: "MainState", **kwargs) -> Dict[str, Any]:
        from tableAgent.table_agent.states.base_state import PlanningState
