...
from table_agent.toolkits.tool_manager import get_tool_manager, ToolManager
...
from . import tableProcessing_agents
...
def create_table_agent(name: str, tool_manager: Optional[ToolManager] = get_tool_manager(), **kwargs):
    """
    创建表格处理模式 Agent
    
    表格处理模式适用于需要对表格数据进行分析、清洗和转换的场景，
    支持基于表格内容的复杂推理和操作。
    ...
    Returns:
        Agent: 配置为表格处理模式的代理角色实例
    
    Examples:
        >>> agent = create_table_agent("table_analyzer", max_retries=7)
        >>> agent = create_table_agent("data_cleaner", model_name="gpt-4", temperature=0.2)
    """
